...
# 2021 raster gave 85.2% and 85.3% overall. Producer's and user's accuracies meet
# the per-class target of at least 85% for every class EXCEPT THREE, stated to be
# regionally lower: 5 (low-growing woody plants), 8 (lichens and mosses) and 9
# (non- and sparsely vegetated). The reasons given are fuzzy class definition,
# limited spectral-temporal separability and sparse reference data.
#
# Class 5 is the maquis. The weakest class of the product is the one that burns.
# By how much is not public -- the ATBD was read and carries no per-class figure,
# and the validation reports are announced as forthcoming. So every vegetation
# row below is `ambiguous`, and fev_fuel_profile() is how the doubt gets settled
# locally against LiDAR HD rather than argued about.
#
