...
A tile runs 120 to 260 MB, measured on two Maures tiles on 2026-08-17 —
the burnt one is half the size of the unburnt one, because there is less
vegetation to return from. Five hundred tiles cover a massif, so a
departmental job is a hundred-gigabyte download that will be interrupted
at least once. Files already on disk with a plausible size are therefore
skipped rather than refetched, which makes re-running the same call the
way to resume.
